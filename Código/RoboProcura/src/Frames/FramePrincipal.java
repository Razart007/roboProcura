...
public class FramePrincipal extends JFrame{
	...
	private void inicializaComponentes(){
		...
		editar.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				if (linhaInicial == -1){
					JOptionPane.showMessageDialog(null,"É necessário preciso abrir um arquivo para editar", "Erro", JOptionPane.ERROR_MESSAGE);
				}
				else {
					editando = true;
					editarAmbiente();
		...
		jBuAmbienteOk.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if((linhaRobo != -1) && (linhaObjetivo != -1)){
					...
					JOptionPane.showMessageDialog(null,"É necessário escolher o estado inicial e o final", "Erro", JOptionPane.ERROR_MESSAGE);
				}
				editando = false;
			}
		});
	...
	private class eventosB implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			if (editando){
				System.out.println("Entrou no método do botão");
				switch (tipoDado){
				case "ambiente":
